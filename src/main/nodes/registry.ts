...
export class NodeRegistry {
  ...
  async load(nodeDefinitionsDir: string): Promise<void> {
    ...
    await Promise.all(
      files.filter((f) => f.endsWith('.json')).map((f) => this.loadFile(path.join(nodeDefinitionsDir, f)))
    );
...
export function validate(data: unknown): string[] {
  ...
  if (d.command_template && d.command_js) errors.push('Cannot have both "command_template" and "command_js"');

  // Syntax-check JS fields at load time so typos surface immediately
  if (typeof d.command_js === 'string') {
    try {
      new Function('params', d.command_js);
    } catch (e) {
      errors.push(`command_js syntax error: ${e}`);
    }
  }
  if (typeof d.compute_js === 'string') {
    try {
      new Function('params', d.compute_js);
    } catch (e) {
      errors.push(`compute_js syntax error: ${e}`);
    }
  }
