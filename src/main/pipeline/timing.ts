...
export class TimingCollector {
  ...
  beginImage(file: string): {
    ...
    return {
      magick: (ms) => { entry.magickMs += ms },
      fileCheck: (ms) => { entry.fileCheckMs = ms },
      copy: (ms) => { entry.copyMs = ms },
      done: (ms) => { entry.totalMs = ms; this._currentBatch?.images.push(entry) },
    }
  }
  ...
  endBatch(outputDir: string | null): void {
    ...
    let avgMs = 0, minMs = 0, maxMs = 0, p95Ms = 0
    if (imgs.length > 0) {
    ...
    const run: BatchRun = { startedAt: b.startedAt, imageCount: b.imageCount, totalMs, setupMs: b.setupMs, startupMs: b.startupMs, images: imgs, avgMs, minMs, maxMs, p95Ms }
  ...
  endImport(totalMs: number): void {
    ...
    const avgIdentify = slowImgs.length ? Math.round(slowImgs.reduce((s, i) => s + i.identifyMs, 0) / slowImgs.length) : 0
    const thumbImgs = [...fastImgs, ...slowImgs]
    const avgThumb = thumbImgs.length ? Math.round(thumbImgs.reduce((s, i) => s + i.thumbnailMs, 0) / thumbImgs.length) : 0
