...
describe('buildNodeData', () => {
  ...
  it('seeds params from def defaults', () => {
    const def = makeDef({
      params: [{ name: 'sigma', type: 'float', label: 'Sigma', default: 2.5, widget: 'slider' }],
    });
    const data = buildNodeData(def);
...
describe('expandNodeData', () => {
  it('merges saved params over definition defaults', () => {
    const def = makeDef({
      params: [{ name: 'sigma', type: 'float', label: 'Sigma', default: 2.0, widget: 'slider' }],
    });
...
describe('firstMatchingHandle', () => {
  ...
  it('matches param port by type on input side', () => {
    const def = makeDef({
      params: [{ name: 'sigma', type: 'float', label: 'Sigma', default: 1, widget: 'slider' }],
    });
  ...
  it('matches readonly param as output port', () => {
    const def = makeDef({
      params: [{ name: 'result', type: 'bool', label: 'Result', default: false, widget: 'checkbox', readonly: true }],
    });
