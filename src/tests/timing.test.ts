...
beforeEach(() => { t = new TimingCollector() })
...
describe('batch recording', () => {
  ...
  it('records per-image entries via beginImage', () => {
    ...
    e1.fileCheck(3); e1.magick(100); e1.copy(2); e1.done(105)
    const e2 = t.beginImage('b.png')
    e2.fileCheck(4); e2.magick(200); e2.copy(3); e2.done(207)
    t.endBatch(null)
    const run = t.batchHistory[0]
  ...
  it('computes avg/min/max from per-image totals', () => {
    ...
    for (const ms of [100, 200, 300]) {
      ...
      e.fileCheck(0); e.magick(ms); e.copy(0); e.done(ms)
    }
    t.endBatch(null)
    const run = t.batchHistory[0]
  ...
  it('magick accumulates across multiple calls', () => {
    ...
    e.magick(50)
    e.magick(75)
    e.fileCheck(0); e.copy(0); e.done(125)
    t.endBatch(null)
