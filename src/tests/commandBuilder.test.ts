...
describe('buildCommandArgs', () => {
  ...
  it('interpolates a param value', () => {
    const d = def({ id: 'x', command_template: '-quality {{quality}}', params: [
      { name: 'quality', label: 'Quality', type: 'int', widget: 'slider', default: 90 },
    ] })
  ...
  it('handles params record value of 0 (falsy) correctly', () => {
    const d = def({ id: 'x', command_template: '-threshold {{val}}', params: [
      { name: 'val', label: 'Val', type: 'float', widget: 'slider', default: 50 },
    ] })
