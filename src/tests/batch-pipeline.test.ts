...
describe('executeBatch', () => {
  ...
  it('returns zero counts for an empty image list', async () => {
    ...
    const graph = makeGraph(
      [{ id: 'workflow-input', type: 'workflow-input', position: { x: 0, y: 0 }, data: { label: 'Input', definitionId: 'workflow-input', params: {} } },
       { id: 'workflow-output', type: 'workflow-output', position: { x: 0, y: 0 }, data: { label: 'Output', definitionId: 'workflow-output', params: {} } }],
      [{ id: 'e1', source: 'workflow-input', target: 'workflow-output', sourceHandle: 'out-0', targetHandle: 'in-0' }]
    )
    const registry = makeRegistry({})
    const result = await executeBatch(graph, [], null, 'overwrite', registry, noop, () => false)
  ...
  it('counts gate-suppressed image as skipped', async () => {
    ...
    const graph = makeGraph(
      [
        { id: 'workflow-input', type: 'workflow-input', position: { x: 0, y: 0 }, data: { label: 'Input', definitionId: 'workflow-input', params: {} } },
        { id: 'gate-1', type: 'gate', position: { x: 0, y: 0 }, data: { label: 'Gate', definitionId: 'gate-def', params: { condition: false } } },
        { id: 'workflow-output', type: 'workflow-output', position: { x: 0, y: 0 }, data: { label: 'Output', definitionId: 'workflow-output', params: {} } },
      ],
      [
    ...
    const result = await executeBatch(
      graph, ['/img/a.png'], null, 'overwrite', registry, noop, () => false
    )
