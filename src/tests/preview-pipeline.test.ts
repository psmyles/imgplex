...
describe('executePreview', () => {
  ...
  it('format_convert node triggers image re-encode', async () => {
    ...
    const graph = makeGraph(
      [
        ...
        { id: 'conv-1', type: 'conv', position: { x: 0, y: 0 }, data: { label: 'Convert', definitionId: 'format-convert-def', params: { format: 'JPEG', quality: 85 } } },
  ...
  it('bypassed node (_enabled=false) copies through without spawning magick', async () => {
    ...
    const graph = makeGraph(
      [
        { id: 'workflow-input', type: 'workflow-input', position: { x: 0, y: 0 }, data: { label: 'Input', definitionId: 'workflow-input', params: {} } },
        { id: 'resize-1', type: 'resize', position: { x: 0, y: 0 }, data: { label: 'Resize', definitionId: 'resize-def', params: { _enabled: false } } },
        { id: 'workflow-output', type: 'workflow-output', position: { x: 0, y: 0 }, data: { label: 'Output', definitionId: 'workflow-output', params: {} } },
      ],
      [
