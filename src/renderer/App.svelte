<script lang="ts">
  ...
  $effect(() => {
    window.ipcRenderer
      .invoke(IPC.REGISTRY_GET_ALL)
      .then((defs: NodeDefinition[]) => {
        definitions = defs;
      })
      .catch((err: unknown) => {
        console.error('[App] Failed to load node registry:', err);
      });
    const onUpdated = (_e: unknown, defs: NodeDefinition[]) => {
      definitions = defs;
    };
  ...
  function startDrag(handle: Handle, e: MouseEvent) {
    ...
    function onMove(ev: MouseEvent) {
      ...
      if (handle === 'left')
        leftWidth = Math.max(cssNum('--left-panel-min'), Math.min(cssNum('--left-panel-max'), w0Left + dx));
      if (handle === 'right')
        rightWidth = Math.max(cssNum('--right-panel-min'), Math.min(cssNum('--right-panel-max'), w0Right - dx));
      if (handle === 'bottom')
        filmstripHeight = Math.max(cssNum('--filmstrip-min'), Math.min(cssNum('--filmstrip-max'), h0Film - dy));
      if (handle === 'inspector')
        inspectorPct = Math.max(
          cssNum('--inspector-min'),
          Math.min(cssNum('--inspector-max'), p0Insp + dy / rightColAvail)
        );
    }
...
{#if updateState}
  <UpdateModal state={updateState} onClose={() => (updateState = null)} />
{/if}
...
{#if graphStore.batchSummaryOpen && graphStore.batchSummary}
  <BatchSummaryModal
    onClose={() => {
      graphStore.batchSummaryOpen = false;
    }}
  />
{/if}
