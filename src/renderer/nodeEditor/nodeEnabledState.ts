 ...
 */
export function isNodeEffectivelyEnabled(nodeId: string, nodes: MinNode[], edges: MinEdge[]): boolean {
  const node = nodes.find((n) => n.id === nodeId);
  if (!node) return true;

  const bypassEdge = edges.find((e) => e.target === nodeId && e.targetHandle === 'param-in-_enabled');
