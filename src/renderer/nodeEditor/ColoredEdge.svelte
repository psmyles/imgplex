<script lang="ts">
  ...
  const [edgePath] = $derived(getBezierPath({ sourceX, sourceY, sourcePosition, targetX, targetY, targetPosition }));
...
<BaseEdge path={edgePath} style="stroke: {baseColor}; stroke-width: {strokeW}; filter: {edgeFilter}" />
