<script lang="ts">
  ...
  async function runBatch() {
    ...
    const outputDir = connectedFolderPath !== null ? connectedFolderPath : outputPath === 'custom' ? customPath : null;
    ...
    try {
      const result = (await window.ipcRenderer.invoke(
        ...
        overwrite,
        generateLog
      )) as { processed: number; skipped: number; failed: number; errors?: string[] };
...
<div class="param-row">
  ...
  <Dropdown
    ...
    onchange={(v) => {
      graphStore.setParam(selectedNode.id, 'outputMode', v);
    }}
  />
</div>
...
{:else}
  ...
  <div class="param-row">
    ...
    <Dropdown
      ...
      onchange={(v) => {
        graphStore.setParam(selectedNode.id, 'overwrite', v);
      }}
    />
  </div>
  ...
  <div class="run-section" class:hidden={!IS_ELECTRON}>
    ...
    {#if graphStore.batchDone && !graphStore.batchRunning && graphStore.batchSummary}
      <button
        class="summary-btn"
        class:has-errors={!!graphStore.batchError}
        onclick={() => {
          graphStore.batchSummaryOpen = true;
        }}
      >
