<script lang="ts">
  ...
  function rgbToMode(m: ColorMode, r: number, g: number, b: number): number[] {
    switch (m) {
      ...
      case 'HSV':
        return [hue, sat, bri]; // use internal state to preserve hue stability
      case 'LAB':
  ...
  function modeToRgb(m: ColorMode, vals: number[]): [number, number, number] {
    switch (m) {
      case 'RGB01':
        return [c01(vals[0]), c01(vals[1]), c01(vals[2])];
      case 'RGB255':
        return [c01(vals[0] / 255), c01(vals[1] / 255), c01(vals[2] / 255)];
      case 'HSV':
        return hsvToRgb(Math.max(0, Math.min(360, vals[0] || 0)), c01(vals[1]), c01(vals[2]));
      case 'LAB':
        return labToRgb(vals[0] || 0, vals[1] || 0, vals[2] || 0);
      case 'CMYK':
        return cmykToRgb(c01(vals[0]), c01(vals[1]), c01(vals[2]), c01(vals[3]));
    }
  }
  ...
  function sqMove(e: PointerEvent) {
    if (sqPicking) updateSquare(e);
  }
  function sqUp() {
    sqPicking = false;
  }
...
<style>
  ...
  .ch-num::-webkit-inner-spin-button {
    -webkit-appearance: none;
    margin: 0;
  }
