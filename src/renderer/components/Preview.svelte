<script lang="ts">
  ...
  function buildPreviewGraph(sfNodes: Node[], sfEdges: Edge[], userPreviewId: string | null): PreviewGraphResult {
    ...
    const extraNodes = sfNodes.filter((n) => {
      ...
      const imageInEdges = sfEdges.filter((e) => e.target === n.id && (e.targetHandle ?? '').startsWith('in-'));
    ...
    const propEdges = sfEdges.filter(
      (e) => (extraIds.has(e.source) || extraIds.has(e.target)) && !filteredEdges.some((fe) => fe.id === e.id)
    );
  ...
  $effect(() => {
    ...
    untrack(() => {
      graphStore.activePreviewNodeId = effectivePreviewId;
    });
