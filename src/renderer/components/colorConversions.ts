...
export function c255(v: number): number {
  return Math.round(c01(v) * 255);
}
...
export function rgbToLab(r: number, g: number, b: number): [number, number, number] {
  ...
  const Y = 0.2126729 * slin(r) + 0.7151522 * slin(g) + 0.072175 * slin(b);
  const Z = 0.0193339 * slin(r) + 0.119192 * slin(g) + 0.9503041 * slin(b);
...
export function labToRgb(L: number, a: number, b: number): [number, number, number] {
  ...
  return [
    ...
    c01(sdelin(-0.969266 * X + 1.8760108 * Y + 0.041556 * Z)),
